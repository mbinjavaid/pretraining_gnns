...
and assign functional group encodings to the atoms (nodes), as well as properties to the molecules (graphs)."""
...
def mol_to_graph_data_obj_simple(molecule, smarts_queries):
    """
    Converts rdkit mol object to graph Data object required by the pytorch
    geometric package. NB: Uses simplified atom and bond features, and represent
    as indices
    :param molecule: rdkit mol object
    :param smarts_queries: list of subsructure queries
    :return: graph data object with the attributes: x, edge_index, edge_attr
    """
    # Atoms:
    atom_fg_membership = []
    fg_mapping = atom_functional_group_mapping(molecule, smarts_queries)
    # Create a tensor of size (num_atoms, num_functional_groups) where each row is a one-hot vector
    # indicating which functional groups that atom belongs to.
    # create an empty tensor to store both atom numbers and many-hot functional group encodings:
    x = torch.empty(molecule.GetNumAtoms(), len(smarts_queries) + 2, dtype=torch.long)
    for idx, atom in enumerate(molecule.GetAtoms()):
        atom_type = types[atom.GetSymbol()]
        if atom.GetIdx() in fg_mapping:
            atom_fg_membership.append(fg_mapping[atom.GetIdx()])
        else:
            atom_fg_membership.append([])
        atom_fg_onehot = torch.zeros(len(smarts_queries) + 1).long()  # keep an extra dimension for masked token
        for fg in atom_fg_membership[idx]:
            atom_fg_onehot[fg] = 1
        # Check if atom_fg_onehot has all zeros, if so, then make its last entry a 1 (to denote unspecified /
        # unknown fg):
        if not torch.any(atom_fg_onehot):
            atom_fg_onehot[-1] = 1
        # set x[idx] as the combination of atom_type and atom_fg_onehot:
        x[idx] = torch.cat((torch.LongTensor([atom_type]), atom_fg_onehot), 0)
    ...
    data = Data(x=x, x_original=x, edge_index=edge_index)
...
class QM9(InMemoryDataset):
    ...
    def raw_file_names(self):
        return 'qm9_preprocessed.csv', 'smarts_queries.txt'
    ...
    def process(self):
        ...
        # initialize RingInfo properties
        ...
        for mol_id, molecule in enumerate(df['mol']):
            ...
            for idx, atom in enumerate(molecule.GetAtoms()):
                ...
                # print("idx and atom idx: " + str(idx) + ", " + str(atom.GetIdx()))
                # if idx != atom.GetIdx():
                #     raise ValueError('idx not equal to atom.GetIdx()!')
...
class DownstreamDataset(InMemoryDataset):
    ...
    def raw_file_names(self):
        ...
        return self.dataset_name + '_preprocessed.csv', 'smarts_queries.txt'

    @property
    def processed_file_names(self):
        if self.dataset_name is None:
            raise ValueError('dataset_name must be specified')
        return self.dataset_name + '_emb.pt'
...
def graph_data_obj_to_nx_simple(data):
    ...
    for i in range(num_atoms):
        atomic_num_idx = atom_features[i][0]
        fg_membership = atom_features[i][1:]
        ...
        if self.dataset == 'zinc_standard_agent':
            ...
            for i in range(len(smiles_list)):
                ...
                if rdkit_mol != None:  # ignore invalid mol objects
                    ...
                    data = mol_to_graph_data_obj_simple(rdkit_mol, smarts_queries)
                    # # manually add mol id
