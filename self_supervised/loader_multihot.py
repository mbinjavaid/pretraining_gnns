...
def mol_to_graph_data_obj_simple(molecule, smarts_queries):
    ...
    x2 = torch.empty(molecule.GetNumAtoms(), len(smarts_queries))
    for idx, atom in enumerate(molecule.GetAtoms()):
        atom_types.append(types[atom.GetSymbol()])
        if atom.GetIdx() in fg_mapping:
            atom_fg_membership.append(fg_mapping[atom.GetIdx()])
        else:
            atom_fg_membership.append([])
        atom_fg_onehot = torch.zeros(len(smarts_queries))
        for fg in atom_fg_membership[idx]:
            atom_fg_onehot[fg] = 1
        x2[idx] = atom_fg_onehot

    x1 = F.one_hot(torch.tensor(atom_types), num_classes=len(types))
    # All atom features (atom type and functional group membership) are concatenated into a single tensor
