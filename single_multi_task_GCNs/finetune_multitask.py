...
def main():
    ...
    pretrained_models = glob.glob('models_gcn/multitask/*.pt')
    ...
    for pretrained_model in pretrained_models:
        ...
        global_pooling = pretrained_model_params[0]
        ...
        for target_property_downstream in args.target_properties_downstream:
            ...
            print("Fine tuning on the dataset " + target_property_downstream + " with pretraining on all QM9 properties -- "
                                                                               "Pooling: " + global_pooling, flush=True)
            ...
            print("Test Stats for property " + target_property_downstream + " pretrained on all QM9 properties "
                                                                            "(" + pretrained_model_name + ") over"
                                                                            " all 10 folds:\n", flush=True)
            ...
            all_folds_val_rmses = []
            all_folds_val_maes = []
            df.loc[len(df)] = ['multitask', global_pooling, hidden_dim, num_layers, batch_norm,
                               target_property_downstream, average_rmse, median_rmse, minimum_rmse,
                               std_rmse, cv]

        if os.path.exists('results/single_multi_task/gcn_multitask.csv'):
            df.to_csv('results/single_multi_task/gcn_multitask.csv', index=False, mode='a', header=False)
        else:
            df.to_csv('results/single_multi_task/gcn_multitask.csv', index=False)
