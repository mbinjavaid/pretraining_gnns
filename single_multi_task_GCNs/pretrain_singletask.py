...
def main():
    ...
    for n in range(len(target_properties_pretraining)):
        ...
        # Only consider the current (nth) target property:
        dataset.data.y = dataset.data.y[:, n:n + 1]

        print('---- Target: Training of single-task model for QM9 property: ' + target_properties_pretraining[n] +
              ' ----', flush=True)
        ...
        # Check number of tasks (e.g. single-task or multi-task)
        ...
        for epoch in range(1, args.epochs + 1):
            ...
            print(f'Pre-training Property: {target_properties_pretraining[n]}, Epoch: {epoch}, Training Loss: {train_loss}',
                  flush=True)
        ...
        if not osp.exists('models_gcn/singletask'):
            os.makedirs('models_gcn/singletask')
        model_path = ('models_gcn/singletask/' + target_properties_pretraining[n] + '_' + args.global_pooling + '_' + str(args.hidden_dim) +
